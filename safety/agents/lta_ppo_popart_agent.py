...
class LTAPPOPOPARTAgent:
    ...
    def __init__(self,
        ...
        if critic.__str__() == "PopArtCritic":
            self.critic = critic
        else:
            Exception("Critic must be a PopArtCritic")
    ...
    def update(self, batch: dict, pretrain = False):
        ...
        if not self.recompute_adv:
            with torch.no_grad():
                ...
                # update the mu and sigma of critic
                self.critic.update_parameters(b_targets)
                # compute normalized targets
                b_norm_targets = (b_targets - self.critic.mu) / self.critic.sigma
        ...
        for i in range(update_epochs):
            ...
            for start in range(0, len(b_inds), minibatch_size):
                ...
                mb_norm_targets = b_norm_targets[mb_inds]
                ...
                if self.critic_first:
                    mb_critic_results = self.update_critic_mb(mb_obs, mb_norm_targets, mb_targets)
                    if not pretrain:
                        mb_actor_results = self.update_actor_mb(mb_obs, mb_actions, mb_log_probs, mb_advantages, mb_interventions)
                else:
                    if not pretrain:
                        mb_actor_results = self.update_actor_mb(mb_obs, mb_actions, mb_log_probs, mb_advantages, mb_interventions)
                    mb_critic_results = self.update_critic_mb(mb_obs, mb_norm_targets)
    ...
    #     elif self.critic.type() == "PopArtCritic":
    #         value, norm_value = self.critic(nn_obs)
    #     else:
    ...
    #     """
    #     if self.critic.type() == "PopArtCritic":
    #         value, norm_value = self.critic(nn_obs)
    #     else:
    #         norm_value = self.critic(nn_obs)
    #     return norm_value  #
    ...
    def compute_targets(self, rewards, nn_obs, next_nn_obs, dones):
        ...
        with torch.no_grad():
            ...
            b_values, b_norm_values = self.critic(nn_obs)

            # get values of next observations
            b_next_value, b_next_norm_value = self.critic(next_nn_obs)
    ...
    def compute_TD_advantages(self, rewards, values, next_val):
        with torch.no_grad():
            ...
            for t in reversed(range(len(adv))):
                ...
                adv[t] = rewards[t] - self.eta - values[t] + next_value
    ...
    def update_critic_mb(self, mb_obs, mb_norm_targets, mb_targets):
        ...
        # === Logging === #
        # Compute errors
        critic_errors = (new_norm_values - mb_norm_targets)
        true_errors = (new_values - mb_targets)

        explained_norm_variance = 1 - (torch.var(critic_errors) / torch.var(mb_norm_targets))
        explained_variance = 1 - (torch.var(true_errors) / torch.var(mb_targets))
    ...
    def update_b_eta(self, rewards, nn_obs):
        ...
        with torch.no_grad():
            # updating b
            values, _ = self.critic(nn_obs)  # nn_obs = batch["nn_obs"]
